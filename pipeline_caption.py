...
def carousel_slide_rules() -> str:
    ...
    return (
        ...
        "ATTRIBUTION: Name the person or account behind the content only once across the whole carousel — use their name from speaker_name when provided, otherwise the @username handle (e.g. '@unitingnow argues', '@grahamformaine says'). Every later reference to them must NOT repeat the name or handle: use 'he'/'she' if their gender is known, otherwise 'they' (e.g. 'they said', 'they also said', 'they added'), or just state the fact/quote with no attribution at all. Never use 'the video', 'the reel', 'the clip', 'the speaker', 'the creator', or 'the account' as a stand-in for a person or source.\n"
        "BANNED PHRASES: the argument is, the claim is, the warning is, the headline says, the headline reads, the article says, the article states, the article reads, the speaker says, the clip says, the video says, the reel says, the post says, according to the post, the caption reads, the speaker, the transcript, the creator said, the comment says, the post points out, the post notes, this matters because, why this matters, the carousel, the carousel opens, the carousel begins, the first slide, the second slide, the third slide, the opening slide, opens with, this slide, the post opens, the video, the reel, the clip\n\n"
