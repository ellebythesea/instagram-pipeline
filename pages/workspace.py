...
def _delete_workspace_grid_row(row: dict) -> None:
    """Move the row the grid's delete control names, then reload the workspace."""
    row_num = row["row_number"]
    try:
...
def _rerun_workspace(tab: str | None = None) -> None:
    ...
    # Increment a global nonce so every popover label changes and Streamlit re-renders them closed.
    st.session_state["_workspace_rerun_nonce"] = st.session_state.get("_workspace_rerun_nonce", 0) + 1
    st.rerun()
...
if active_section_tab == "Home":
    ...
    # and deletes straight away. Clear the URL first, or the delete replays on the
    # next rerun.
